...
def main(number_1: int, number_2: int):
    # url = f'http://0.0.0.0:5000/add/{number_1}/{number_2}/'  # Replace with your API endpoint URL
    
    # try:
    #     response = requests.get(url)
    #     response.raise_for_status()  # Raise an exception if the request was unsuccessful
    #     data = response.json()  # Convert the response to JSON
    #     return f"the is = {data['sum']}"  # Return the response data as the API response
    # except requests.exceptions.RequestException as e:
    #     return f"An error occurred: {str(e)}"
    return {"sum is": "number_1 + number_2"}
